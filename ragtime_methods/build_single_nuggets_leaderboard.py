...
async def async_generate(response_path: pathlib.Path, builder: LeaderboardBuilder, processed_topic_ids: list):
    ...
    for request in requests_to_process:
        ...
        for item in response.responses:
            ...
            try:
                result = await ask_once(prompt)
                score = int(result)
                scores.append(score)
            except Exception as e:
                print(f"Failed scoring nugget for {request.request_id}: {e}")
                continue
...
async def main():
    ...
    leaderboard.write(pathlib.Path("Response_as_Individual_Nuggets.output.eval.txt"), format="ir_measures")
